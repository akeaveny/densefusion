...
for i = 1:numel(keyframes)
    ...
    % load posecnn poses
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % posecnn results is offset by 1    
    posecnn_filename = fullfile(opt.posecnn_results(), sprintf('%06d.mat',  i - 1));
    posecnn_results = load(posecnn_filename);
    ...
    for j = 1:numel(gt.cls_indexes)
        ...
        % network result
        roi_index = find(posecnn_results.rois(:, 2) == cls_index);
        if isempty(roi_index) == 0
            RT = zeros(3, 4);

            % pose after ICP refinement
            RT(1:3, 1:3) = quat2rotm(posecnn_results.poses_icp(roi_index, 1:4));
            RT(:, 4) = posecnn_results.poses_icp(roi_index, 5:7);
