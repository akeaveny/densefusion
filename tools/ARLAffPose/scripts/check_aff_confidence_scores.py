...
def main():
    ...
    # MATLAB RESULTS DIR.
    file_path = config.AFF_EVAL_FOLDER_DF_ITERATIVE + '/*.mat'
    ...
    for obj_id in range(1, config.NUM_OBJECTS+1):
        ...
        plt.title(f'Object Id: {obj_id}, {affpose_dataset_utils.map_obj_id_to_name(obj_id)}', fontsize=10)
        plt.xlabel('$Confidence$', fontsize=10)
        plt.ylabel('$Frequency$', fontsize=10)
        plt.xlim(0, 1.15)
        # plotting configs
        _color =  affpose_dataset_utils.obj_color_map(idx=obj_id)
        ...
        print(f'Object Id: {obj_id},\t\t Num: {len(row_idxs)}'
              ...
              f'\t\t Name: {affpose_dataset_utils.map_obj_id_to_name(obj_id)}'
              )

        # plot data.
        # plt.plot(range(len(row_idxs)), _pred_c, color=color, label=f'{affpose_dataset_utils.map_obj_id_to_name(obj_id)}')
        plt.hist(_pred_c, bins=10, color=color, label=f'{affpose_dataset_utils.map_obj_id_to_name(obj_id)}')
    # plt.show()
