...
def get_obj_bbox(mask, obj_id, img_width, img_length, border_list, show_mask=False):

    ####################
    ####################

    if show_mask:
        cv2.imshow('mask', cv2.cvtColor(arl_affpose_dataset_utils.colorize_aff_mask(mask), cv2.COLOR_BGR2RGB))
        cv2.waitKey(0)
